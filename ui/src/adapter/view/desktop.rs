...
pub async fn run(sender: Sender<Event>, receiver: Receiver<Event>) {
    // Determine language (very lightweight): RU if LANG starts with "ru", otherwise EN
    let is_ru = std::env::var("LANG")
        .unwrap_or_default()
        .to_lowercase()
        .starts_with("ru");
    // Translations for menu/tray strings
    let tr_service: &str = if is_ru { "Сервис" } else { "Service" };
    let tr_restart: &str = if is_ru { "Перезапустить" } else { "Restart" };
    let tr_start: &str = if is_ru { "Запустить" } else { "Start" };
    let tr_stop: &str = if is_ru { "Остановить" } else { "Stop" };
    let tr_logs: &str = if is_ru { "Логи" } else { "Logs" };
    let tr_config: &str = if is_ru { "Конфигурация" } else { "Configuration" };
    let tr_exit_gui: &str = if is_ru { "Выход" } else { "Exit GUI" };
    ...
    let menu = Menu::new()
        .add_submenu(Submenu::new(
            tr_service,
            Menu::new()
                .add_item(CustomMenuItem::new("restart", tr_restart))
                .add_native_item(MenuItem::Separator)
                .add_item(CustomMenuItem::new("start", tr_start))
                .add_item(CustomMenuItem::new("stop", tr_stop)),
        ))
        .add_submenu(Submenu::new(
            tr_logs,
            Menu::new()
    ...
    let tray = SystemTray::new().with_menu(
        SystemTrayMenu::new()
            ...
            .add_item(CustomMenuItem::new("stop", tr_stop))
            .add_native_item(SystemTrayMenuItem::Separator)
            .add_item(CustomMenuItem::new("exit", tr_exit_gui)),
    );
    ...
    let product = if is_ru { "Сервер RustDesk" } else { "RustDesk Server" };
